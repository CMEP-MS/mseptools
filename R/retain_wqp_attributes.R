...
#'   Portal (WQP) download function `dataRetrieval::readWQPdata()`,
#'   containing the full set of WQP attributes.
#' @param data_reduced A data frame derived from `data_full` with empty
#'   columns removed (e.g., via `janitor::remove_empty("cols")`), which
#'   has lost some or all of the original attributes.
#'
#' @return A data frame with the same columns as `data_reduced` and the
